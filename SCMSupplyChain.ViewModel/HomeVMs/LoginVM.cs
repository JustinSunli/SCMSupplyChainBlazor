...
namespace SCMSupplyChain.ViewModel.HomeVMs
{
    public class LoginVM : BaseVM
    {
        ...
        {
            //根据用户名和密码查询用户
            var rv = await DC.Set<FrameworkUser>().Where(x => x.ITCode.ToLower() == ITCode.ToLower() && x.Password == Utils.GetMD5String(Password)).Select(x => new { itcode = x.ITCode, id = x.GetID() }).SingleOrDefaultAsync();
